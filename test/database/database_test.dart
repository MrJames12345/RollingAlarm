...
void main() {
  ...
  group('RA_Database Tests', () {
    ...
    test('RoutineState transitions work correctly', () async {
      // 1. Insert a routine
      final routineId = await db.insertRoutine(RoutinesCompanion(
      ...
      await db.insertRoutineState(RoutineStatesCompanion(
    ...
    test('LogEntry writes with correct TimeSinceLastDismissal', () async {
      final routineId = await db.insertRoutine(RoutinesCompanion(
        ...
        DriftCompensationTypeCode: Value(DriftCompensationTypeCodeEnum.InitialRing.index),
      ));

      // First dismissal (no previous dismissal, so null or 0)
      final firstDismissalTime = DateTime(2026, 1, 1, 8, 0, 0);
      await db.insertLogEntry(LogEntriesCompanion(
        RoutineId: Value(routineId),
      ...
      final secondDismissalTime = firstDismissalTime.add(const Duration(hours: 4, minutes: 30));
      final secondsDiff = secondDismissalTime.difference(firstDismissalTime).inSeconds;
    ...
    test('Second connection to the same file observes first connection writes', () async {
      ...
      try {
        ...
        final routineId = await db1.insertRoutine(RoutinesCompanion(
        ...
        await db2.insertRoutineState(RoutineStatesCompanion(
          RoutineId: Value(routineId),
          IsRinging: const Value(true),
          CurrentSnoozeCount: const Value(0),
        ));
    ...
    test('softDeleteRoutine clears live state so ringing watches stop', () async {
      final routineId = await db.insertRoutine(RoutinesCompanion(
    ...
    test('ensureRoutineState is idempotent under unique RoutineId', () async {
      ...
      final all = await (db.select(db.routineStates)
            ..where((s) => s.RoutineId.equals(routineId)))
          .get();
      expect(all.length, equals(1));
    });

    test('updateRoutineState CAS returns 0 when IsRinging precondition fails',
        () async {
      final routineId = await db.insertRoutine(RoutinesCompanion(
    ...
    test('openForIsolate applies WAL, busy_timeout, and synchronous pragmas',
        () async {
      ...
      try {
        ...
        final journal = await opened.customSelect('PRAGMA journal_mode;').get();
